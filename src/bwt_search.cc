...
void BWTSearch::SearchExact(BWT &bwt, const char *str, AlignType &pos)
{
    ...
    pos.q_begin = i + 1;
    pos.q_end = strlen(str) - 1;
    pos.cost = 0;
    pos.bwt_begin = range.first;
    pos.bwt_end = range.second;
    return;
}
...
)
{
    ...
    if(len <= 0)
    {
        cerr << "BWTSearch::Search: Searching empty sequence. An empty list will be returned." << endl;
        return;
    }
    ...
    vector<AlignType> cand_pos;
    cand_pos.resize(len);
    vector<queue<AlignType> > record_range;
    record_range.resize(len);
    for(int i = 0; i < cand_pos.size(); ++ i)
    {
        //cout << "=======================" << endl;
        cand_pos[i].bwt_begin = 0;
        cand_pos[i].bwt_end = bwt_size;
        ...
        for(int j = 0; j <= i; ++ j)
        {
            ...
            cand_pos[j].bwt_begin = x;
            cand_pos[j].bwt_end = y;
            if(cand_pos[j].bwt_begin < cand_pos[j].bwt_end && cand_pos[j].q_end - cand_pos[j].q_begin + 1 >= min_len)
            {
                ...
                if(record_range[j].size() >= flank_len)  record_range[j].pop();
    ...
    {
        if(it->empty() || (long long int) it->back().bwt_end - (long long int) it->back().bwt_begin + 1 > max_hits)
        {
            continue;
        }
        while(!it->empty())
...
)
{
    ...
    //sort(all_pos.begin(), all_pos.end(), SortAlignTypeByBWTPos);
    vector<MatchType> all_match(num_pos);
    ...
    for(auto it = all_pos.begin(); it != all_pos.end(); ++ it)
    {
        ...
        for(BWTIDX i = it->bwt_begin; i < it->bwt_end; ++ i)
        {
            ...
            all_match[idx].q_begin = it->q_begin;
            all_match[idx].q_end = it->q_end;
            ...
            //cerr << "Before ungapped extension:  " << all_match[idx].q_begin << "  " << all_match[idx].q_end << " " << all_match[idx].str_pos << endl;
            ...
            all_match[idx].cigar = new char [50];
            sprintf(all_match[idx].cigar, "%d%c", all_match[idx].str_len, 'M');

            //stringstream ss; ss << all_match[idx].str_len; ss << "M";
            //all_match[idx].cigar = ss.str();

            //cerr << all_match[idx].cigar << endl;
        ...
        for(int ii = idx_chunk_begin; ii < idx; ++ ii)
        {
            ...
            if(all_match[ii].score >= max_score
                    //&& ref_begin.find(all_match[ii].str_pos) == ref_begin.end()
              )
            {
                ...
                //cerr << "pushed:  " << all_match[ii].str_pos << endl;
                extended_frag.push_back(all_match[ii]);
                //cerr << all_match[ii].cigar << endl;
                //cerr << "Gapped Extended:  " << all_match[ii].score << " " << all_match[ii].q_begin << " " << all_match[ii].q_end << " " << all_match[ii].str_pos << endl;
            }
            else
            {
...
)
{
    ...
    ExtInfo init;
    init.range.first = 0;
    init.range.second = bwt.GetSize();
    init.q_pos = len - 1;
    init.cost = 0;
    init.cost_bound = bound[len - 1];
    Enqueue(candidate, init);
    pos.bwt_begin = 0;
    pos.bwt_end = bwt.GetSize();
    pos.q_begin = pos.q_end = len - 1;
    pos.cost = cost;
    ...
    while(!candidate.empty())
    {
        ...
        {
            pos.q_begin = candidate.top().q_pos + 1;
            pos.cost = candidate.top().cost;
            pos.bwt_begin = candidate.top().range.first;
            pos.bwt_end = candidate.top().range.second;
        }
            ...
            for(k = current.ivB_[i].first; k < current.ivB_[i].second; ++ k)
            {
                ...
                for(j = i; j < n; ++ j)
                {
                    ...
                    if(r1.second - r1.first >= 1 && r2.second - r2.first <= 0)
                    {
                        ...
                        next.PushA(-1, -1);
                        next.PushB(r1.first, r1.second);
                        next.PushLen(current.len_[j]);
                    }
                }
