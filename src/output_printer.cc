    ...
    for(int i = 0; i < result.size() && begin + i < upper; ++ i)
    {
        for(int j = 0; j < result[i].size(); ++ j)
        {
            ...
            for(int k = 0; k < result[i][j].targets.size(); ++ k)
            {
                ...
                ++ result[i][j].targets[k].begin;
                ++ result[i][j].targets[k].end;
    ...
    for(int i = 0; i < result.size() && begin + i < upper; ++ i)
    {
        ...
        for(int j = 0; j < result[i].size(); ++ j)
        {
            ...
            for(int k = 0; k < result[i][j].targets.size(); ++ k)
            {
                ...
                if(cigar_len < 2)
                {
                    cerr << "Output error: super short cigar: " << cigar_len << "  " << result[i][j].targets[k].cigar << endl;
                }
...
////////////////////////////////////////
    ...
    int num_chunks;
    in_fh.read((char *) &num_chunks, sizeof(int));
    int current_chunk = 0;
    //cerr << num_chunks << endl;
    while(!in_fh.eof())
    {
        long long int total_hits;
        in_fh.read((char *) &total_hits, sizeof(long long int));
        if(!in_fh) ExitWithMessage("CLAN:OutputPrinter::ReadEncoded: file could be corrupted. Abort.", 1);
        long long int current_hits = 0;
        while(!in_fh.eof())
        {
            int rid;
            in_fh.read((char *) &rid, sizeof(int));
            if(!in_fh) ExitWithMessage("CLAN:OutputPrinter::ReadEncoded: file could be corrupted. Abort.", 1);
            int num_maps;
            in_fh.read((char *) &num_maps, sizeof(int));
            if(!in_fh) ExitWithMessage("CLAN:OutputPrinter::ReadEncoded: file could be corrupted. Abort.", 1);
            ...
            for(int i = 0; i < num_maps; ++ i)
            {
                ...
                int num_targets;
                in_fh.read((char *) &num_targets, sizeof(int));
                if(!in_fh) ExitWithMessage("CLAN:OutputPrinter::ReadEncoded: file could be corrupted. Abort.", 1);
