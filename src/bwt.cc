...
)
{
    ...
    for(int i = 0; i < size_; ++ i)
    {
        ...
        {
            cout << "BWT::ConstructNoPosFromIndex: unknow character encoutered: " << bwt_[i] << "; abort" << endl;
            exit(1);
        }
    }
    acc_freq_ = new BWTIDX [alphabet_.GetSize() + 2]; // note the trailing character '$'
...
void BWT::LoadPosition(const std::string &file_name)
{
    ...
    if(!in_fh.is_open())
    {
        cout << "BWT::LoadPosition: failed to open BWT position indexing file: " << file_name << "; Abort." << endl;
        exit(1);
    }
...
void BWT::LoadFMIndex(const std::string &file_name)
{
    ...
    if(!in_fh.is_open())
    {
        cout << "BWT::LoadFMIndex: failed to open FM-index file: " << file_name << "; Abort." << endl;
        exit(1);
    }
    ...
    {
        cout << "BWT::LoadFMIndex: size of FM index does not match the text, abort." << endl;
        exit(1);
    }
    // loading the files line by line
    ...
    while(getline(in_fh, line))
    {
        ...
        {
            cout << "BWT::LoadFMIndex: invalid character found in FM index: " << c << "; abort." << endl;
            exit(1);
        }
        if(gap_size != range.size() - 2)
        {
            cout << "BWT::LoadFMIndex: FM index size does not match text length; abort." << endl;
            exit(1);
        }
    ...
    for(int i = 0; i < alphabet_.GetSize(); ++ i)
    {
        ...
        {
            cout << "BWT::LoadFMIndex: character not found in FM index: " << c << "; abort." << endl;
            exit(1);
        }
    }
    if(loaded_char.find(DELIM) == loaded_char.end())
    {
        cout << "BWT::LoadFMIndex: DELIM not found in FM index; abort." << endl;
        exit(1);
    }
    in_fh.close();
    return;
