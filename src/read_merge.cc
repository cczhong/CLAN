...
)
{
    for(int i = 0; i < num_ref_; ++ i)
    {
        ...
        GPosType current_pos;
        current_pos.sid = i;
        current_pos.begin = -1;
        current_pos.strand = true;
        current_pos.coverage = 0;
        int uncovered_len = 0;
        ...
        for(BWTIDX j = 0; j < ref_len_[i]; ++ j)
        {
            ...
            if(coverage_pos_[i][j] > 0)
            {
                ...
                {
                    current_pos.end = j;
                    current_pos.coverage += coverage_pos_[i][j];
                }
                else
                {
                    island_open = true;
                    current_pos.begin = current_pos.end = j;
                    current_pos.coverage = coverage_pos_[i][j];
                    uncovered_len = 0;
                }
            }
    ...
    for(int i = 0; i < num_ref_; ++ i)
    {
        ...
        for(BWTIDX j = 0; j < ref_len_[i]; ++ j)
        {
            ...
            if(coverage_rev_[i][j] > 0)
            {
                ...
                {
                    current_pos.end = j;
                    current_pos.coverage += coverage_rev_[i][j];
                }
                else
                {
                    island_open = true;
